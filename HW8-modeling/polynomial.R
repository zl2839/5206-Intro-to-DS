...
library(tidyverse)
data_with_id<-read_tsv("data/polyfit.tsv")%>%mutate(id=row_number())
...
quad_resid_plot<-
  ggplot(data_with_quad_resid,aes(x,resid))+
  geom_ref_line(h=0)+
  geom_point()+
  labs(y = "Residuals",
       title = "The residuals show a secondary pattern that is not captured")
...
## Do not modify this line!
library(purrr)
my_model<-function(k){lm(y~poly(x,k,raw=TRUE),data=train_data)}
model_pred_resid<-
  tibble(k=1:10)%>%
  mutate(model=map(k,my_model))%>%
  mutate(data=map(model,~all_data%>%add_predictions(.x)%>%add_residuals(.x)))%>%
  unnest(data)%>%
  select(-model)
...
model_mse_penalized<-
  ...
  group_by(k,split)%>%
  nest()%>%
  mutate(error=data%>%map(~my_mse_penalized(.x,k=k,lambda=c(0, 0.1, 0.4))))%>%
  unnest(error)%>%
  select(-data)
