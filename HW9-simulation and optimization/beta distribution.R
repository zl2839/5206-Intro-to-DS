...
## Do not modify this line!
par_beta = function(x){
  return(optim(c(1,1),obj_beta_factory(x))$par)
}
alpha1 = par_beta(x)[1]
beta1 = par_beta(x)[2]
...
opt = optim(c(1,1),nll_beta, x=x,
            hessian = T,
            lower = 0, method = 'L-BFGS-B')
esti = opt$hessian %>% solve() %>% sqrt() %>% diag()
alpha2 = opt$par[1]
beta2 = opt$par[2]
lower = c(alpha2,beta2) - 1.96*esti
upper = c(alpha2,beta2) + 1.96*esti
...
#stat_function(fun = partial(dbeta,shape1 = alpha1,shape2 = beta1),  aes(color = "MOM"))+
library(ggplot2)
as.data.frame(x)
