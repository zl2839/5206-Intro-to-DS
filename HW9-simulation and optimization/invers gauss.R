...
obj_invgauss = function(par,x){
  moments = mom_invgauss(par)
  t = c(mean(x),var(x))
  differences = t - moments
  sse = sum(differences^2)
  return(sse)
}
obj_invgauss(c(1,1),c(1,2,3))
...
g1 = ggplot(random_data_tbl,aes(sample=random_data))+
  ...
  labs(title = "The sample seems to distribute along the straight line",
       ...
       x = "Theoretical values",y = "Actual values")+theme_light()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
...
nll_invgauss = function(par,x){
  -sum(dinvgauss(x,par[1],par[2],log = TRUE))
}
set.seed(100)
seed2 = .Random.seed

fit = optim(c(1,1),nll_invgauss,x = random_data,
            hessian = TRUE,
            lower = 0, method = 'L-BFGS-B')
library(tidyverse)
se = fit$hessian %>% solve() %>% sqrt() %>% diag()  
par2 = fit$par
lower = par2 - 1.96*se
upper = par2 + 1.96*se
...
#
## Do not modify this line!
g2 = ggplot(random_data_tbl,aes(x = random_data))+
  geom_histogram(aes(y = ..density..),bins = 15)+
  geom_density(aes(color = 'Data'))+
  stat_function(fun =partial(dinvgauss,
                             mean = par2[1],
                             shape = par2[2]), 
                aes(color = "MOM")) +
  stat_function(fun =partial(dinvgauss,
