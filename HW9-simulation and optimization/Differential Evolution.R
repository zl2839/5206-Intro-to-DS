...
newton_bowl = optim(par0,fn = obj_bowl ,method = 'BFGS')

set.seed(100)
seed2 = .Random.seed 
newton_ackley = optim(par0,fn = obj_ackley ,method = 'BFGS')
...
library(tidyverse)
value_of_params = de_ackley$member$bestmemit %>% as_tibble() %>% rename(x = par1, y = par2) %>% mutate(iteration = (1:200), objective = f_ackley(x,y))
...
g1 = ggplot(value_of_params,aes(x,y))+
  geom_line(aes(color = objective))+
...
#    To do this, you can use:
#      - `geom_line()` to  line in which intercept of y should
...
#
## Do not modify this line!
value_of_params_pivot = value_of_params %>% pivot_longer(-3)

g2 = ggplot(value_of_params_pivot,aes(iteration,value,color = name))+
  geom_line()+scale_x_log10()+geom_hline(yintercept = 0,linetype = 2)+
  labs(title = "Evolution of parameters on 200 iterations",
       ...
       x = "Iterations", y = "Value of parameters", 
       color = "Name")+
  theme_light() + 
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
