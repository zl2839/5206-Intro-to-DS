...
data = read_csv('C:/Users/15783/Desktop/CU/GR5206/HW9/fish.csv')
...
library(ggplot2)
g1 = ggplot(data, aes(count))+geom_histogram()+
  labs(title = "Count of Fish Caught",
       subtitle = "A great many visitors caught no fish at all.",
       x = "Fish Caught (n)",y = "Count (n)")+
  theme_light()
...
## Do not modify this line!
g2 = ggplot(data,aes(x = count))+
  stat_ecdf()+
  labs(title = "Empirical Distribution of Fish Caught",
       subtitle = "Distribution is right-skewed, suggesting a Poisson distribution with a low mean.",
       x = "Fish Caught (n)", y = "ECDF") +theme_light()
...
nll_pois_factory= function(x){
  ...
  xbar = mean(x)
  function(par){
    -(y*log(par[2]+(1-par[2])*exp(1)^(-par[1]))+(n-y)*log(1-par[2])-
        (n-y)*par[1] + n* xbar * log(par[1]))
  }
}
nll_pois_factory(cnt)(c(4, 0.5))
x = c(11, 12, 2, 8, 12, 8, 1, 16, 16, 18)
nll_pois_factory(x)(c(4.15042616892606, 0.694281864678487))
...
par_pois2 = function(x){
  ...
  se = fisher_info %>% sqrt() %>% diag()
  lower = fit$par - 1.96*se
  upper = fit$par + 1.96*se
  t = tibble('parameter' = c('lambda','pi'),
             'value' = fit$par,
             'se' = se,
             'lower' = lower,
             'upper' = upper)
...
cnt_pois_mle = fit_mle %>% pull(value)
dparams_mle = list(mu = t(cnt_pois_mle)[,1], 
                   sigma = t(cnt_pois_mle)[,2])
